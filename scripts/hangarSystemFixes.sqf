...
// Function to add cleanup button to hangar UI
// NEW: Adds manual cleanup button and status display
HANGAR_fnc_addCleanupUI = {
    params ["_display"];
    
    if (isNull _display) exitWith {
        diag_log "HANGAR UI: Cannot add cleanup UI - display is null";
        false
    };
    
    // Define position for cleanup button - at top-right corner
    private _buttonY = safezoneY + (safezoneH * 0.12);
    private _buttonH = safezoneH * 0.05;
    private _buttonW = safezoneW * 0.12;
    private _buttonX = safezoneX + safezoneW - _buttonW - (safezoneW * 0.01);
    
    // Create cleanup button
    private _cleanupBtn = _display ctrlCreate ["RscButton", 9890];
    _cleanupBtn ctrlSetPosition [
        _buttonX,
        _buttonY,
        _buttonW,
        _buttonH
    ];
    _cleanupBtn ctrlSetText "System Cleanup";
    _cleanupBtn ctrlSetBackgroundColor [0.4, 0.2, 0.6, 0.8];
    _cleanupBtn ctrlSetTextColor [1, 1, 1, 1];
    _cleanupBtn ctrlSetTooltip "Perform a comprehensive cleanup of the hangar system";
    
    // Add click handler
    _cleanupBtn ctrlAddEventHandler ["ButtonClick", {
        // Run cleanup and get results
        private _results = [false] call HANGAR_fnc_cleanupHangarSystem;
        _results params ["_totalIssues", "_aircraftIssues", "_pilotIssues", "_referenceIssues"];
        
        // Show detailed results dialog
        private _msg = format [
            "Hangar System Cleanup Complete\n\n" +
            "Total Issues Fixed: %1\n" +
            "• Aircraft Issues: %2\n" +
            "• Pilot Issues: %3\n" +
            "• Reference Issues: %4\n\n" +
            "Refreshing display...",
            _totalIssues, _aircraftIssues, _pilotIssues, _referenceIssues
        ];
        
        hint _msg;
        
        // If issues were fixed, refresh the display
        if (_totalIssues > 0) then {
            ...
            if (!isNil "HANGAR_fnc_updateAircraftInfo") then {
                call HANGAR_fnc_updateAircraftInfo;
            };
            
            if (!isNil "HANGAR_fnc_updateActionButtonStates") then {
                call HANGAR_fnc_updateActionButtonStates;
            };
        };
    }];
    
    _cleanupBtn ctrlCommit 0;
    
    // Add to UI controls array
    HANGAR_uiControls pushBack _cleanupBtn;
    
    // Create status indicator near the button
    private _statusIndicator = _display ctrlCreate ["RscStructuredText", 9891];
    _statusIndicator ctrlSetPosition [
        _buttonX,
        _buttonY + _buttonH + (safezoneH * 0.01),
        _buttonW,
        _buttonH * 0.6
    ];
    
    // Set initial status
    private _statusColor = "#8cff9b"; // Green
    private _statusText = "System Healthy";
    
    if (isNil "HANGAR_lastCleanupTime") then {
        HANGAR_lastCleanupTime = 0;
    };
    
    if (time - HANGAR_lastCleanupTime > 600) then {
        _statusColor = "#ffcc00"; // Yellow
        _statusText = "Status Check Needed";
    };
    
    _statusIndicator ctrlSetStructuredText parseText format [
        "<t align='center' size='0.8' color='%1'>%2</t><br/>" +
        "<t align='center' size='0.7'>Monitored: %3</t>",
        _statusColor,
        _statusText,
        if (!isNil "HANGAR_monitoringActive" && {HANGAR_monitoringActive}) then {"Active"} else {"Inactive"}
    ];
    
    _statusIndicator ctrlSetBackgroundColor [0, 0, 0, 0.5];
    _statusIndicator ctrlCommit 0;
    
    // Store reference for updates
    HANGAR_statusIndicator = _statusIndicator;
    HANGAR_uiControls pushBack _statusIndicator;
    
    // Create monitoring toggle button
    private _monitorToggle = _display ctrlCreate ["RscButton", 9892];
    _monitorToggle ctrlSetPosition [
        _buttonX,
        _buttonY + _buttonH + (safezoneH * 0.01) + (_buttonH * 0.6) + (safezoneH * 0.01),
        _buttonW,
        _buttonH * 0.5
    ];
    
    // Set button text based on monitoring status
    if (!isNil "HANGAR_monitoringActive" && {HANGAR_monitoringActive}) then {
        _monitorToggle ctrlSetText "Stop Monitoring";
        _monitorToggle ctrlSetBackgroundColor [0.6, 0.2, 0.2, 0.8];
    } else {
        _monitorToggle ctrlSetText "Start Monitoring";
        _monitorToggle ctrlSetBackgroundColor [0.2, 0.6, 0.2, 0.8];
    };
    
    // Add click handler
    _monitorToggle ctrlAddEventHandler ["ButtonClick", {
        params ["_ctrl"];
        
        if (!isNil "HANGAR_monitoringActive" && {HANGAR_monitoringActive}) then {
            // Stop monitoring
            [] call HANGAR_fnc_stopSystemMonitoring;
            
            // Update button
            _ctrl ctrlSetText "Start Monitoring";
            _ctrl ctrlSetBackgroundColor [0.2, 0.6, 0.2, 0.8];
            
            // Update status indicator
            if (!isNil "HANGAR_statusIndicator" && {!isNull HANGAR_statusIndicator}) then {
                HANGAR_statusIndicator ctrlSetStructuredText parseText format [
                    "<t align='center' size='0.8' color='#ffcc00'>Monitoring Stopped</t><br/>" +
                    "<t align='center' size='0.7'>Monitored: Inactive</t>"
                ];
            };
        } else {
            // Start monitoring
            ...
            // Update button
            _ctrl ctrlSetText "Stop Monitoring";
            _ctrl ctrlSetBackgroundColor [0.6, 0.2, 0.2, 0.8];
            
            // Update status indicator
            if (!isNil "HANGAR_statusIndicator" && {!isNull HANGAR_statusIndicator}) then {
                HANGAR_statusIndicator ctrlSetStructuredText parseText format [
                    "<t align='center' size='0.8' color='#8cff9b'>Monitoring Active</t><br/>" +
                    "<t align='center' size='0.7'>Monitored: Active</t>"
                ];
            };
        };
    }];
    
    _monitorToggle ctrlCommit 0;
    HANGAR_uiControls pushBack _monitorToggle;
    
    // Update timestamp
    HANGAR_lastCleanupTime = time;
    
    diag_log "HANGAR UI: Cleanup UI elements added successfully";
    true
};

// Hook to inject this into the main UI creation
if (!isNil "HANGAR_fnc_createCustomUI_original") then {
    // Original already backed up
    diag_log "HANGAR UI: Hook already established";
} else {
    // Back up original function
    HANGAR_fnc_createCustomUI_original = HANGAR_fnc_createCustomUI;
    
    // Create new function that calls the original and adds our element
    HANGAR_fnc_createCustomUI = {
        // Call original function
        _this call HANGAR_fnc_createCustomUI_original;
        
        // Add our cleanup UI
        [findDisplay 312] call HANGAR_fnc_addCleanupUI;
    };
    
    diag_log "HANGAR UI: Function hook established for cleanup UI";
};
