...
systemChat format ["Formation Column called with %1 selections", count _selections];
...
if (count _selections > 1) then {
    ...
    // Set formation for each group
    {
        ...
        systemChat format ["%1 switching to COLUMN formation", groupId _x];
    } forEach _groups;
} else {
    ...
    if (!isNull _group) then {
        ...
        systemChat format ["%1 switching to COLUMN formation", groupId _group];
    };
};